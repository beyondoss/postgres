...
//!   * client commands arrive as RESP3 arrays of bulk strings:
//!       HELLO 3       — handshake; server responds with a minimal map
//!       PING          — server replies "+PONG\r\n"
//!       WATCH [SINCE <lsn>] — register subscriber and stream push frames; SINCE
//!                             is parsed and ignored (slot replays from
//!                             confirmed_flush_lsn server-side)
//!       UNWATCH       — server replies "+OK\r\n" and closes the connection
//!   * server pushes (RESP3 `>` prefix) are 2-element arrays of bulk strings:
...
/// Bind `0.0.0.0:port` and accept connections forever, spawning one OS thread
/// per accepted connection. Exits the process on bind failure (callers treat
/// the listener as fatal infrastructure).
pub fn serve(port: u16, subs: Subscribers) -> ! {
    ...
    eprintln!("cdc resp listening on 0.0.0.0:{port}");

    loop {
        match listener.accept() {
            Ok((stream, _)) => {
                let subs = Arc::clone(&subs);
                std::thread::spawn(move || handle_conn(stream, subs));
            }
...
fn handle_conn(stream: TcpStream, subs: Subscribers) {
...
fn stream_events(mut writer: TcpStream, subs: Subscribers) {
    // Disable read timeout: this socket is now write-only from our side.
    let _ = writer.set_write_timeout(Some(Duration::from_secs(30)));

    let (tx, rx): (SyncSender<Arc<[u8]>>, Receiver<Arc<[u8]>>) = sync_channel(SUBSCRIBER_CAPACITY);
    {
    ...
    if write_push(&mut writer, &[b"change", b"ready"]).is_err() {
        return;
    }
    ...
    loop {
        match rx.recv_timeout(HEARTBEAT_INTERVAL) {
            Ok(msg) => {
                if write_push(&mut writer, &[b"change", &msg]).is_err() {
                    return;
                }
            }
...
fn read_cmd(r: &mut BufReader<TcpStream>) -> io::Result<Cmd> {
    ...
    Ok(match verb.as_slice() {
        b"HELLO" => Cmd::Hello,
        b"PING" => Cmd::Ping,
        // SINCE is accepted (parts[1] == "SINCE", parts[2] == lsn) but ignored:
        // logical replication slots replay from confirmed_flush_lsn server-side,
        // so client-supplied resume points would only narrow that, not extend it.
        b"WATCH" => Cmd::Watch,
...
mod tests {
    ...
    #[test]
    fn parses_watch_since_ignores_lsn() {
        // SINCE arg is parsed past but does not change the resulting Cmd.
        assert_eq!(
            parse(b"*3\r\n$5\r\nWATCH\r\n$5\r\nSINCE\r\n$10\r\n1/23456780\r\n").unwrap(),
            Cmd::Watch
        );
    ...
    fn hello_then_ping_handshake() {
        ...
        let handle = std::thread::spawn(move || handle_conn(server, empty_subs));
