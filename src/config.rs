...
/// Parent directory for per-worker PgBouncer peer sockets. Each so_reuseport worker
/// gets its own `pgb-<peer_id>` subdirectory here so their `.s.PGSQL.5432` sockets
/// don't collide; the sockets exist only so peers can forward cancels to each other.
/// On tmpfs (`/run`), so it survives a supervisor handoff but is recreated on cold boot.
pub const PGB_PEER_DIR: &str = "/run/beyond-pg";

/// PgBouncer `peer_id` (1-based; `0` disables peering) for a worker's stable name.
/// Worker 0 keeps the legacy name "pgbouncer" (→ 1); extras are "pgbouncer-N" (→ N+1).
/// Peer ids are stable across restarts so the shared `[peers]` map is fixed and the
/// scaler never has to rewrite config as it grows/shrinks the live worker set.
pub fn pgb_peer_id(name: &str) -> usize {
    match name.strip_prefix("pgbouncer-") {
        Some(n) => n.parse::<usize>().map(|n| n + 1).unwrap_or(1),
        None => 1, // "pgbouncer"
    }
}

/// Per-worker unix socket directory for the given `peer_id` (`PGB_PEER_DIR/pgb-<id>`).
pub fn pgb_peer_socket_dir(peer_id: usize) -> String {
    format!("{PGB_PEER_DIR}/pgb-{peer_id}")
}

/// Config file path for the worker with the given `peer_id`. Worker 1 keeps the
/// canonical `/etc/pgbouncer/pgbouncer.ini`; extras get `pgbouncer-<id>.ini`.
pub fn pgbouncer_ini_path(peer_id: usize) -> String {
    if peer_id <= 1 {
        PGBOUNCER_INI_PATH.to_string()
    } else {
        format!("/etc/pgbouncer/pgbouncer-{peer_id}.ini")
    }
}

/// pgbouncer.ini for the worker with the given `peer_id` (1-based): static base +
/// boot-computed pool sizing + TLS termination + so_reuseport peering.
///
/// When the box can run more than one worker (`pgbouncer_max_workers > 1`), each
/// worker gets a DISTINCT `unix_socket_dir` + unique `peer_id` and an identical
/// `[peers]` map covering every possible worker (1..=max). That map is what lets a
/// query-cancel request the kernel routes (via so_reuseport) to a worker that isn't
/// holding the session be forwarded over the peer socket to the one that is —
/// without it, misdirected cancels are silently dropped. The map is pre-declared
/// for the full worker cap so the scaler never rewrites config; forwarding to a
/// peer that isn't currently running simply fails, exactly as a dropped cancel
/// would have before, so pre-declaring idle peers is safe.
///
/// Single-worker boxes (`max == 1`) can never misroute a cancel, so peering is
/// omitted and `unix_socket_dir` stays empty (no client-facing unix socket; clients
/// use TCP :5432, the direct Postgres path is :5433).
pub fn pgbouncer_ini(
    ...
    // so_reuseport peering — cross-worker query cancellation.
    let max_workers = pgbouncer_max_workers(vcpus);
    if max_workers > 1 {
        out.push_str(
        ...
        out.push_str(&format!(
            "unix_socket_dir = {}\n",
            pgb_peer_socket_dir(peer_id)
        ));
        out.push_str(&format!("peer_id = {peer_id}\n"));
        out.push_str("\n[peers]\n");
        for id in 1..=max_workers {
            out.push_str(&format!("{id} = host={}\n", pgb_peer_socket_dir(id)));
        }
    } else {
        // Single-worker box: no peer can ever hold a foreign session, so keep the
        // client-facing unix socket disabled (clients use TCP :5432).
        out.push_str("\nunix_socket_dir =\n");
    }
    out
}
...
/// Generates `05-pitr.conf` when `BEYOND_PG_RECOVERY_TARGET_TIME` is set.
///
/// PITR replays WAL fetched from the WAL sink (`beyond-pg-sink`) — the single
/// redo source — via `restore_command` (same curl form as `replica_conf`).
/// `recovery_target_time` makes Postgres replay up to the target on a cloned
/// snapshot volume and then promote. `wal_sink` is the sink base URL.
pub fn pitr_conf(wal_sink: &str, recovery_target_time: Option<&str>) -> String {
    let sink = wal_sink.trim_end_matches('/');
    let mut out =
        String::from("# Generated by beyond-pg. Do not edit — overwritten on every boot.\n\n");
    // %f = WAL segment filename, %p = destination path. curl -f fails on HTTP
    // errors so Postgres retries; -s suppresses progress.
    out.push_str(&format!("restore_command = 'curl -f -s {sink}/%f -o %p'\n"));
...
mod tests {
    ...
    fn filter_preload_keeps_only_installed_libs() {
        ...
        assert!(
            !out.contains("beyond_auth"),
            "beyond_auth not installed: {out}"
        );
        assert!(
            !out.contains("beyond_queue"),
            "beyond_queue not installed: {out}"
        );
    ...
    fn filter_preload_all_present_is_unchanged() {
        ...
        let out = filter_shared_preload_libraries(conf, pkglibdir);
        assert!(
            out.contains("shared_preload_libraries = 'pg_stat_statements,auto_explain,pg_cron'")
        );
    }
    ...
    fn pgbouncer_ini_appends_tls_keys() {
        ...
        let out = pgbouncer_ini(&tls, 8 * 1024 * 1024 * 1024, 4, 1);
        assert!(
    ...
    fn pgbouncer_scales_with_box() {
        ...
        // 4 vCPU -> max_workers 1: single worker can't misroute a cancel, so peering
        // is off and the client-facing unix socket stays disabled (TCP :5432 only).
        let small = pgbouncer_ini(&tls, 4 * 1024 * 1024 * 1024, 4, 1);
        ...
        assert!(
            ...
            "single-worker box keeps empty unix_socket_dir: {small}"
        );
        assert!(
            !small.contains("[peers]"),
            "no peering on 1-worker box: {small}"
        );
        assert!(
            !small.contains("peer_id ="),
            "no peer_id on 1-worker box: {small}"
        );
        // Mid box: full pool 48 (16 vCPU * 3), not divided across workers.
        let mid = pgbouncer_ini(&tls, 32u64 * 1024 * 1024 * 1024, 16, 2);
        assert!(
        ...
        let big = pgbouncer_ini(&tls, 256u64 * 1024 * 1024 * 1024, 64, 1);
    ...
    #[test]
    fn pgb_peer_id_maps_stable_names() {
        assert_eq!(pgb_peer_id("pgbouncer"), 1);
        assert_eq!(pgb_peer_id("pgbouncer-1"), 2);
        assert_eq!(pgb_peer_id("pgbouncer-7"), 8);
    }

    #[test]
    fn pgbouncer_ini_path_keeps_canonical_for_worker_one() {
        assert_eq!(pgbouncer_ini_path(1), PGBOUNCER_INI_PATH);
        assert_eq!(pgbouncer_ini_path(2), "/etc/pgbouncer/pgbouncer-2.ini");
        assert_eq!(pgbouncer_ini_path(8), "/etc/pgbouncer/pgbouncer-8.ini");
    }

    #[test]
    fn pgbouncer_ini_emits_peering_on_multiworker_box() {
        let tls = TlsConfig {
            source: TlsSource::SelfSigned,
            cert: PathBuf::from("/c"),
            key: PathBuf::from("/k"),
        ...
        // 16 vCPU -> max_workers 4. Worker 2's config must carry its own identity
        // plus the FULL [peers] map (all 4 workers) so cancels route to any peer.
        let w2 = pgbouncer_ini(&tls, 32u64 * 1024 * 1024 * 1024, 16, 2);
        assert!(w2.contains("peer_id = 2"), "worker 2 identity: {w2}");
        assert!(
            w2.contains("unix_socket_dir = /run/beyond-pg/pgb-2"),
            "worker 2 socket dir: {w2}"
        );
        assert!(w2.contains("[peers]"), "peers section: {w2}");
        for id in 1..=4 {
            assert!(
                w2.contains(&format!("{id} = host=/run/beyond-pg/pgb-{id}")),
                "peer {id} declared: {w2}"
            );
        }
        // The map covers exactly the worker cap — no peer 5 on a 4-worker box.
        assert!(!w2.contains("5 = host="), "no over-declared peer: {w2}");
        // Every worker on the box shares an identical [peers] map (only identity differs).
        let w1 = pgbouncer_ini(&tls, 32u64 * 1024 * 1024 * 1024, 16, 1);
        let peers_of = |s: &str| s.split("\n[peers]\n").nth(1).unwrap().to_string();
        assert_eq!(peers_of(&w1), peers_of(&w2), "shared [peers] map");
        assert!(w1.contains("peer_id = 1"));
    }
    ...
    fn pitr_conf_restore_command_only() {
        let conf = pitr_conf("http://10.0.0.5:9000", None);
        assert!(
            conf.contains("restore_command = 'curl -f -s http://10.0.0.5:9000/%f -o %p'"),
            "restore_command wrong or missing: {conf}"
        );
    ...
    fn pitr_conf_with_recovery_target() {
        let conf = pitr_conf("http://10.0.0.5:9000", Some("2026-05-14 03:00:00"));
        assert!(
            conf.contains("restore_command = 'curl -f -s http://10.0.0.5:9000/%f -o %p'"),
    ...
    fn pitr_conf_strips_trailing_slash() {
        let conf = pitr_conf("http://10.0.0.5:9000/", None);
        assert!(
            conf.contains("http://10.0.0.5:9000/%f"),
            "trailing slash not stripped: {conf}"
        );
        assert!(!conf.contains(":9000//%f"), "double slash present: {conf}");
    }

    #[test]
    fn pitr_conf_escapes_single_quotes_in_target_time() {
        let conf = pitr_conf("http://s/p", Some("2026-05-14 03:00:00.000+00''00"));
        assert!(
